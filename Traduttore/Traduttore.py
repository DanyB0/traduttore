...
class Window(tk.Frame):
    ...
    def widgets(window):
        ...
        window.titolo_da_tradurre = tk.Label(window, text=" TESTO DA TRADURRE ", font=("Helvetica", 11), fg="#0462f9")
        ...
        window.titolo_tradotto = tk.Label(window, text=" TRADUZIONE ", font=("Helvetica", 11), fg="red")
        ...
        window.testo_da_tradurre = tk.Entry(window, textvariable=window.nome_da_tradurre)
        ...
        window.traduci = tk.Button(window, text="TRADUCI", font=("Helvetica", 11), fg="#8e18b2", relief="flat", command=window.traduzione2, activebackground="#282623")
        ...
        window.scelta = tk.Listbox(window, bg="light blue", width=30, fg="blue", highlightcolor="blue")
        window.scelta.grid(row=1, column=0, sticky="nw")
        #Casella di scelta a destra
        window.scelta_2 = tk.Listbox(window, bg="pink", width=30, fg="red", highlightcolor="red")
        window.scelta_2.grid(row=1, column=2, sticky="nw")
        #Bottone per ascoltare la parola/frase inserita
        window.audio_1 = tk.Button(window, relief="flat", command=window.leggi_1, pady=10)
        window.audio_1.grid(row=3, column=0, sticky="sew")
        window.audio_1.config(background="#282623", image=window.audio1, activebackground="#282623")
        #Bottone per ascoltare la parola/frase tradotta
        window.audio_2 = tk.Button(window, relief="flat", command=window.leggi_2, pady=10)
        window.audio_2.grid(row=3, column=2, sticky="sew")
        window.audio_2.config(background="#282623", image=window.audio2, activebackground="#282623")
        #Bottone per la guida
        window.info = tk.Button(window, relief="flat", command=window.informazioni, pady=10)
        window.info.grid(row=3, column=1, sticky="sew")
        window.info.config(background="#282623", image=window.infor, activebackground="#282623")
        #Developer
        window.dani = tk.Label(window, text="DanyB0", font=("Helvetica", 7), fg="#12e8bd", bg="#282623").grid(row=3, column=2, sticky="se")
        ...
        window.scelta.bind("<Button-1>", window.prendi_lingua_1) #Button-1 è il tasto destro del mouse
        window.scelta_2.bind("<Button-1>", window.prendi_lingua_2)
        window.testo_da_tradurre.bind("<KeyPress-Return>", window.traduzione) #KeyPress-Return è l'invio
...
#            messagebox.showerror("Lingua non disponibile", "La lingua cercata non è disponibile.")
    ...
    def traduzione(window, event):
        try:
            ...
            messagebox.showerror("Lingua non inserita", "Devi inserire la lingua da cui stai traducendo.")
    ...
    def traduzione2(window):
        try:
            ...
            messagebox.showerror("Lingua non inserita", "Devi inserire la lingua da cui stai traducendo / in cui vuoi tradurre.")
    ...
    def informazioni(window):
        ...
        guida0 = tk.Label(info_finestra, text="GUIDA:", font=("Helvetica", 11), fg="#d1127e", bg="#282623").grid(row=0, column=0, sticky="we")
        guida1 = tk.Label(info_finestra, text="Selezionare la lingua da cui si vuole tradurre dal riquadro blu a sinistra (doppio click)", font=("Helvetica", 11), fg="#12e8bd", bg="#282623").grid(row=1, column=0, sticky="we")
        guida2 = tk.Label(info_finestra, text="Selezionare la lingua in cui si vuole tradurre dal riquadro rosso a destra (doppio click)", font=("Helvetica", 11), fg="#12e8bd", bg="#282623").grid(row=2, column=0, sticky="we")
        guida3 = tk.Label(info_finestra, text="Inserire la parola da tradurre nel riquadro in basso a sinistra (sopra all'altoparlante) e premere invio o la freccia", font=("Helvetica", 11), fg="#12e8bd", bg="#282623").grid(row=3, column=0, sticky="we") 
        guida4 = tk.Label(info_finestra, text="Per ascoltare le parole/frasi premere l'altoparlante (alcune lingue non sono disponibili)", font=("Helvetica", 11), fg="#12e8bd", bg="#282623").grid(row=4, column=0, sticky="we") 
